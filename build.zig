...
pub fn build(b: *std.Build) void {
    ...
    const dep_libssh = b.dependency("acton_zig_libssh", .{
        .target = target,
        .optimize = optimize,
        .WITH_SERVER = true,
    });
    ...
    libActonProject.root_module.linkLibrary(actdep_std.artifact("ActonProject"));
    for (explicit_static_libraries.items) |libActonExplicit| {
        libActonExplicit.root_module.linkLibrary(actdep_std.artifact("ActonProject"));
    }
    libActonProject.root_module.linkLibrary(dep_libssh.artifact("ssh"));
    for (explicit_static_libraries.items) |libActonExplicit| {
        libActonExplicit.root_module.linkLibrary(dep_libssh.artifact("ssh"));
    }
    ...
    if (root_c_files.items.len > 0) {
        ...
        for (root_c_files.items) |entry| {
            ...
            executable.root_module.linkLibrary(actdep_std.artifact("ActonProject"));
            executable.root_module.linkLibrary(dep_libssh.artifact("ssh"));
